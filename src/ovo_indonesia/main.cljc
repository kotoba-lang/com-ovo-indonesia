(ns ovo-indonesia.main
  "Kotodama WASM entrypoint for the Ovo_indonesia clean-room actor (L4) — Clojure port.
  ...
  facts (`ovo_indonesia.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ovo_indonesia")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"ovo_indonesia.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ovo_indonesia-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ovo_indonesia-compat/ready)
