(ns ovo-indonesia.main-test
  "Contract + behavioral test for the ovo-indonesia-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "ovo_indonesia-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'ovo-indonesia.main-test)]
